...
3.  **Queue Message (Input)**: The agent writes a JSON message to the appropriate input queue (e.g., `gap-analyzer-input`) in `aiuc1funcstorage`. The message includes a `CorrelationId` for response matching.
...
Each of the 12 functions has a dedicated input and output queue (24 queues total) in the `aiuc1funcstorage` storage account:
