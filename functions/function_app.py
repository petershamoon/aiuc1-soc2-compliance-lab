...
# Enforcement layer — deterministic AIUC-1 control enforcement
...
def write_output(
    ...
    *,
    ...
    All responses pass through the AIUC-1 enforcement layer, which applies:
    - Mandatory output sanitisation (A006/B009)
    - AI disclosure injection (E016)
    - Enforcement metadata attachment (E017)
    - Cryptographic audit chain recording (E015)
    
    This is architectural enforcement — the LLM cannot bypass it.
    If the enforcement layer fails, falls back to direct output to
    prevent the function from returning an unhandled error.
    """
    try:
        ...
        logger.error("Enforcement layer error, falling back to direct output: %s", e)
...
def gap_analyzer(msg: func.QueueMessage, output: func.Out[str]):
    ...
    # Enforcement layer: input-phase checks (scope boundaries, injection scan)
    blocked, decisions = check_input_enforcement("gap_analyzer", body)
    if blocked:
        write_output(output, build_error_envelope("gap_analyzer",
                     "Request blocked by AIUC-1 enforcement layer",
                     error_code="ENFORCEMENT_BLOCKED", details={"decisions": decisions}),
                     correlation_id, function_name="gap_analyzer", input_payload=body)
        return
    ...
    if cc_error:
        write_output(output, build_error_envelope("gap_analyzer", cc_error,
                     ...
                     function_name="gap_analyzer", input_payload=body)
        return
...
def scan_cc_criteria(msg: func.QueueMessage, output: func.Out[str]):
    ...
    body, correlation_id = parse_queue_msg(msg)
    blocked, decisions = check_input_enforcement("scan_cc_criteria", body)
    if blocked:
        write_output(output, build_error_envelope("scan_cc_criteria",
                     "Request blocked by AIUC-1 enforcement layer",
                     error_code="ENFORCEMENT_BLOCKED", details={"decisions": decisions}),
                     correlation_id, function_name="scan_cc_criteria", input_payload=body)
        return
    cc_category = body.get("cc_category", "").strip().upper()
    cc_error = validate_cc_category(cc_category)
    if cc_error:
        write_output(output, build_error_envelope("scan_cc_criteria", cc_error,
                     error_code="INVALID_CC_CATEGORY"), correlation_id,
                     function_name="scan_cc_criteria", input_payload=body)
        return
    settings = get_settings()
...
def evidence_validator(msg: func.QueueMessage, output: func.Out[str]):
    ...
    body, correlation_id = parse_queue_msg(msg)
    blocked, decisions = check_input_enforcement("evidence_validator", body)
    if blocked:
        write_output(output, build_error_envelope("evidence_validator",
                     "Request blocked by AIUC-1 enforcement layer",
                     error_code="ENFORCEMENT_BLOCKED", details={"decisions": decisions}),
                     correlation_id, function_name="evidence_validator", input_payload=body)
        return
    ...
    write_output(output, build_success_envelope("evidence_validator", result,
                 ...
                 function_name="evidence_validator", input_payload=body)
...
def query_access_controls(msg: func.QueueMessage, output: func.Out[str]):
    ...
    body, correlation_id = parse_queue_msg(msg)
    blocked, decisions = check_input_enforcement("query_access_controls", body)
    if blocked:
        write_output(output, build_error_envelope("query_access_controls",
                     "Request blocked by AIUC-1 enforcement layer",
                     error_code="ENFORCEMENT_BLOCKED", details={"decisions": decisions}),
                     correlation_id, function_name="query_access_controls", input_payload=body)
        return
    ...
    write_output(output, build_success_envelope("query_access_controls", result,
                 aiuc1_controls=["B006", "A003", "C002", "B009", "E015"]), correlation_id,
                 function_name="query_access_controls", input_payload=body)
...
def query_defender_score(msg: func.QueueMessage, output: func.Out[str]):
    ...
    body, correlation_id = parse_queue_msg(msg)
    blocked, decisions = check_input_enforcement("query_defender_score", body)
    if blocked:
        write_output(output, build_error_envelope("query_defender_score",
                     "Request blocked by AIUC-1 enforcement layer",
                     error_code="ENFORCEMENT_BLOCKED", details={"decisions": decisions}),
                     correlation_id, function_name="query_defender_score", input_payload=body)
        return
    ...
    write_output(output, build_success_envelope("query_defender_score", result,
                 ...
                 function_name="query_defender_score", input_payload=body)
...
def query_policy_compliance(msg: func.QueueMessage, output: func.Out[str]):
    ...
    body, correlation_id = parse_queue_msg(msg)
    blocked, decisions = check_input_enforcement("query_policy_compliance", body)
    if blocked:
        write_output(output, build_error_envelope("query_policy_compliance",
                     "Request blocked by AIUC-1 enforcement layer",
                     error_code="ENFORCEMENT_BLOCKED", details={"decisions": decisions}),
                     correlation_id, function_name="query_policy_compliance", input_payload=body)
        return
    ...
    write_output(output, build_success_envelope("query_policy_compliance", result,
                 aiuc1_controls=["B006", "A003", "B009", "E015"]), correlation_id,
                 function_name="query_policy_compliance", input_payload=body)
...
def generate_poam_entry(msg: func.QueueMessage, output: func.Out[str]):
    ...
    body, correlation_id = parse_queue_msg(msg)
    blocked, decisions = check_input_enforcement("generate_poam_entry", body)
    if blocked:
        write_output(output, build_error_envelope("generate_poam_entry",
                     "Request blocked by AIUC-1 enforcement layer",
                     error_code="ENFORCEMENT_BLOCKED", details={"decisions": decisions}),
                     correlation_id, function_name="generate_poam_entry", input_payload=body)
        return
    ...
    write_output(output, build_success_envelope("generate_poam_entry", result,
                 ...
                 function_name="generate_poam_entry", input_payload=body)
...
def run_terraform_plan(msg: func.QueueMessage, output: func.Out[str]):
    ...
    body, correlation_id = parse_queue_msg(msg)
    blocked, decisions = check_input_enforcement("run_terraform_plan", body)
    if blocked:
        write_output(output, build_error_envelope("run_terraform_plan",
                     "Request blocked by AIUC-1 enforcement layer",
                     error_code="ENFORCEMENT_BLOCKED", details={"decisions": decisions}),
                     correlation_id, function_name="run_terraform_plan", input_payload=body)
        return
    settings = get_settings()
    ...
    write_output(output, build_success_envelope("run_terraform_plan", result,
                 ...
                 function_name="run_terraform_plan", input_payload=body)
...
def run_terraform_apply(msg: func.QueueMessage, output: func.Out[str]):
    ...
    body, correlation_id = parse_queue_msg(msg)
    blocked, decisions = check_input_enforcement("run_terraform_apply", body)
    if blocked:
        write_output(output, build_error_envelope("run_terraform_apply",
                     "Request blocked by AIUC-1 enforcement layer",
                     error_code="ENFORCEMENT_BLOCKED", details={"decisions": decisions}),
                     correlation_id, function_name="run_terraform_apply", input_payload=body)
        return
    ...
    write_output(output, build_success_envelope("run_terraform_apply", result,
                 ...
                 function_name="run_terraform_apply", input_payload=body)
...
def git_commit_push(msg: func.QueueMessage, output: func.Out[str]):
    ...
    body, correlation_id = parse_queue_msg(msg)
    blocked, decisions = check_input_enforcement("git_commit_push", body)
    if blocked:
        write_output(output, build_error_envelope("git_commit_push",
                     "Request blocked by AIUC-1 enforcement layer",
                     error_code="ENFORCEMENT_BLOCKED", details={"decisions": decisions}),
                     correlation_id, function_name="git_commit_push", input_payload=body)
        return
    ...
    write_output(output, build_success_envelope("git_commit_push", result,
                 ...
                 function_name="git_commit_push", input_payload=body)
...
def sanitize_output(msg: func.QueueMessage, output: func.Out[str]):
    ...
    body, correlation_id = parse_queue_msg(msg)
    blocked, decisions = check_input_enforcement("sanitize_output", body)
    if blocked:
        write_output(output, build_error_envelope("sanitize_output",
                     "Request blocked by AIUC-1 enforcement layer",
                     error_code="ENFORCEMENT_BLOCKED", details={"decisions": decisions}),
                     correlation_id, function_name="sanitize_output", input_payload=body)
        return
    ...
    write_output(output, build_success_envelope("sanitize_output", result, sanitise=False,
                 ...
                 function_name="sanitize_output", input_payload=body)
...
def log_security_event(msg: func.QueueMessage, output: func.Out[str]):
    ...
    body, correlation_id = parse_queue_msg(msg)
    blocked, decisions = check_input_enforcement("log_security_event", body)
    if blocked:
        write_output(output, build_error_envelope("log_security_event",
                     "Request blocked by AIUC-1 enforcement layer",
                     error_code="ENFORCEMENT_BLOCKED", details={"decisions": decisions}),
                     correlation_id, function_name="log_security_event", input_payload=body)
        return
    ...
    write_output(output, build_success_envelope("log_security_event", result,
                 ...
                 function_name="log_security_event", input_payload=body)
